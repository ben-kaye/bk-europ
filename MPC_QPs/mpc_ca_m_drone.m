...
% Precomputed system with Ts = 0.1. Control inputs desired x vel and y vel.
...
if (VIS_ON)
    ...
    for z = 1 : M        
        ...
        plot(x_hist(nx*(z-1)+1,:), x_hist(nx*(z-1)+2,:), '-^', 'MarkerSize', 5, 'LineWidth', 1, 'Color', color(:,z), 'DisplayName', sprintf('A%d Path Taken',z))
    end
end
...
% rotate ~= 0 -> apply 60deg rotation to etas
    ...
    % insert collision constraints into respective entries for each k in 
    % predictive horison
...
function [ A ] = make_placeholder_A(A, M, N, nx, nu)
    % OSQP does not allow for updating entries which were set to 0. Hence
    % assign placeholders of 1.
